...
int do_ble_scan()
{
  ...
  Serial.println("Synch Timeserver");
  ...
  File file = SPIFFS.open("/contacts.txt", FILE_APPEND);
  ...
  for (int i = 0; i < n; i++)
  {
    if ((foundDevices.getDevice(i).getServiceUUID().toString() == "4fafc201-1fb5-459e-8fcc-c5c9c331914b"))
    {
      ...
      if (foundDevices.getDevice(i).haveRSSI())
      {
        if (strlen((foundDevices.getDevice(i).getAddress()).toString ().c_str()))
        {
          ...
          dataString = String (a_ble_mac) + ";" + String(a_rssi);
  ...
  delay (5000);
  yield();
  File file2 = SPIFFS.open("/contacts.txt");
